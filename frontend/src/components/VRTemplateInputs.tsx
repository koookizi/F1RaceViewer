...
export function VRTemplateInputs({
  ...
  useEffect(() => {
    ...
    const teamsOnly =
      !!requires.teams && !requires.drivers && !requires.allowDriverOrTeam;
    const driversOnly =
      !!requires.drivers && !requires.teams && !requires.allowDriverOrTeam;

    if (teamsOnly && value.selectionMode !== "Teams") {
      onChange({ ...value, selectionMode: "Teams", driverIds: [] });
    } else if (driversOnly && value.selectionMode !== "Drivers") {
      onChange({ ...value, selectionMode: "Drivers", teamIds: [] });
    }
  }, [templateId]); // only when template changes
  ...
  return (
    <div className={`mt-3 space-y-3 ${className}`}>
      ...
      {/* Teams */}
      {requires.teams &&
        (!requires.allowDriverOrTeam || value.selectionMode === "Teams") && (
          <MultiSelect
            options={teamOptions}
            value={value.teamIds}
      ...
      {requires.telemetryCompare && (
        <div className="space-y-2">
          ...
          <div className="alert alert-warning my-4">
            <span className="text-sm">
              Pick 1-2 drivers. If two drivers are selected, the chart compares
              their chosen laps. If one driver is selected, compare two laps
          ...
          <div className="grid grid-cols-2 gap-2 w-100 mb-4">
            <div className="form-control">
              ...
              <select
                ...
                onChange={(e) =>
                  patch({ lapModeA: e.target.value as LapPickMode })
                }
              >
            ...
            <div className="form-control">
              ...
              <select
                ...
                onChange={(e) =>
                  patch({ lapModeB: e.target.value as LapPickMode })
                }
              >
                <option value="Fastest">Fastest lap</option>
                <option value="Manual">Manual lap number</option>
              ...
              {value.lapModeB === "Manual" && (
                <input
                  ...
                  onChange={(e) =>
                    patch({
                      lapNumberB: e.target.value ? Number(e.target.value) : "",
                    })
                  }
                  min={1}
                />
