...
export function TemplateCardPanel({
    ...
    return (
        <div
            className={["rounded-xl border", "bg-base-200 border-base-300/50", "shadow-sm"].join(
                " "
            )}
        >
            <div className="px-4 pt-4 pb-2 flex items-center justify-between">
                <div className="flex flex-col">
                    ...
                    <span className="text-sm font-semibold">{intent || "Select an intent"}</span>
                </div>

                <span className="badge badge-ghost badge-sm">{filtered.length} found</span>
            </div>

            <div className={`px-4 pb-4 ${heightClassName}`}>
                <div
                    className={["h-full overflow-y-auto pr-1", "grid grid-cols-1 gap-3"].join(" ")}
                >
