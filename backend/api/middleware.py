...
class JsonExceptionMiddleware(MiddlewareMixin):
    ...
    def process_exception(self, request, exception):
        ...
        if isinstance(exception, HTTPError):
            ...
            if status == 429:
                payload = {"error": "429", "message": "API - Rate Limit Hit (429). Please retry shortly."}
            ...
            if status == 401:
                return JsonResponse(
                    {"error": "401", "message": "API - Unauthorized (401). Possibly an ongoing session right now. Try again after."},
                    status=401,
                )

            if status == 400:
                return JsonResponse(
                    {"error": "400", "message": "API - Bad Request (400). Query parameters are invalid."},
                    status=400,
                )

            if status == 403:
                return JsonResponse(
                    {"error": "403", "message": "API - Forbidden (403)."},
                    status=403,
                )

            if status == 404:
                return JsonResponse(
                    {"error": "404", "message": "API - Not found (404). Resource not found."},
                    status=404,
                )
